...
public class CommandRegister<T extends MindustryCommand<Player>> {
    ...
    public static void registerDiscordCommands(List<DiscordCommand> commands) {
        Set<String> whitelistedCommands = new HashSet<>();
        if (!allDiscordCommandsEnabled){
            whitelistedCommands = booleans.keySet().stream().filter(k -> k.contains("discord.commands.command-whitelist.")).collect(Collectors.toSet());
        }
        for (var command : commands) {
            if (!whitelistedCommands.isEmpty() && !whitelistedCommands.contains("discord.commands.command-whitelist." + command.name)){
